...
parser.add_argument("--depth", type=int, default=58, help="number of tree depth")
parser.add_argument("--k", type=int, default=20, help="number of tree width")
...
class Predictor:
    def __init__(self):
        # estimator
        checkpoint = torch.load("checkpoint/model_1.pth")
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = SVFModel().to(self.device)
        ...
        self.tree = Tree(self.model, self.config)

    def predict(self, board: Board, color: Stone) -> int:
        scores = self.tree.search(
            board,
            color,
        )
...
class OthelloGame:
    ...
    def render_result(self) -> None:
        print(len(self.predictor.tree.values))
