...
class Board:
    ...
    def key(self, turn: Stone) -> int:
        actions = self.black_actions if turn == Stone.BLACK else self.white_actions
        key = (self.black_board << 128) | (self.white_board << 64) | actions
        return key
...
def gen_random_board(n: int) -> tuple[Board, Stone]:
    while True:
        _n = n
        board = Board()
        turn = Stone.BLACK
